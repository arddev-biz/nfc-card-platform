import { IoCall } from "@react-icons/all-files/io5/IoCall";
import { IoMail } from "@react-icons/all-files/io5/IoMail";
import { IoLocation } from "@react-icons/all-files/io5/IoLocation";
import { IoGlobe } from "@react-icons/all-files/io5/IoGlobe";
import { IoNavigate } from "@react-icons/all-files/io5/IoNavigate";
import { IoRestaurant } from "@react-icons/all-files/io5/IoRestaurant";
import { IoShareSocial } from "@react-icons/all-files/io5/IoShareSocial";
import { IoPerson } from "@react-icons/all-files/io5/IoPerson";
import { IoStar } from "@react-icons/all-files/io5/IoStar";
import { IoLink } from "@react-icons/all-files/io5/IoLink";
import { RiPhoneFill } from "@react-icons/all-files/ri/RiPhoneFill";
import { RiMailFill } from "@react-icons/all-files/ri/RiMailFill";
import { RiMapPinFill } from "@react-icons/all-files/ri/RiMapPinFill";
import { RiGlobalFill } from "@react-icons/all-files/ri/RiGlobalFill";
import { RiDirectionFill } from "@react-icons/all-files/ri/RiDirectionFill";
import { RiRestaurantFill } from "@react-icons/all-files/ri/RiRestaurantFill";
import { RiShareFill } from "@react-icons/all-files/ri/RiShareFill";
import { RiUserFill } from "@react-icons/all-files/ri/RiUserFill";
import { RiStarFill } from "@react-icons/all-files/ri/RiStarFill";
import { RiLinksFill } from "@react-icons/all-files/ri/RiLinksFill";
import { HiPhone } from "@react-icons/all-files/hi/HiPhone";
import { HiMail } from "@react-icons/all-files/hi/HiMail";
import { HiLocationMarker } from "@react-icons/all-files/hi/HiLocationMarker";
import { HiGlobe } from "@react-icons/all-files/hi/HiGlobe";
import { HiMenu } from "@react-icons/all-files/hi/HiMenu";
import { HiShare } from "@react-icons/all-files/hi/HiShare";
import { HiUser } from "@react-icons/all-files/hi/HiUser";
import { HiStar } from "@react-icons/all-files/hi/HiStar";
import { HiLink } from "@react-icons/all-files/hi/HiLink";
import { FiPhone } from "@react-icons/all-files/fi/FiPhone";
import { FiMail } from "@react-icons/all-files/fi/FiMail";
import { FiMapPin } from "@react-icons/all-files/fi/FiMapPin";
import { FiGlobe } from "@react-icons/all-files/fi/FiGlobe";
import { FiNavigation } from "@react-icons/all-files/fi/FiNavigation";
import { FiMenu } from "@react-icons/all-files/fi/FiMenu";
import { FiShare2 } from "@react-icons/all-files/fi/FiShare2";
import { FiUser } from "@react-icons/all-files/fi/FiUser";
import { FiStar } from "@react-icons/all-files/fi/FiStar";
import { FiLink } from "@react-icons/all-files/fi/FiLink";
import { IoCallOutline } from "@react-icons/all-files/io5/IoCallOutline";
import { IoMailOutline } from "@react-icons/all-files/io5/IoMailOutline";
import { IoLocationOutline } from "@react-icons/all-files/io5/IoLocationOutline";
import { IoGlobeOutline } from "@react-icons/all-files/io5/IoGlobeOutline";
import { IoNavigateOutline } from "@react-icons/all-files/io5/IoNavigateOutline";
import { IoRestaurantOutline } from "@react-icons/all-files/io5/IoRestaurantOutline";
import { IoShareSocialOutline } from "@react-icons/all-files/io5/IoShareSocialOutline";
import { IoPersonOutline } from "@react-icons/all-files/io5/IoPersonOutline";
import { IoStarOutline } from "@react-icons/all-files/io5/IoStarOutline";
import { IoLinkOutline } from "@react-icons/all-files/io5/IoLinkOutline";
import { BsPhone } from "@react-icons/all-files/bs/BsPhone";
import { BsEnvelope } from "@react-icons/all-files/bs/BsEnvelope";
import { BsGeoAlt } from "@react-icons/all-files/bs/BsGeoAlt";
import { BsWindow } from "@react-icons/all-files/bs/BsWindow";
import { BsCursor } from "@react-icons/all-files/bs/BsCursor";
import { BsList } from "@react-icons/all-files/bs/BsList";
import { BsBoxArrowUpRight } from "@react-icons/all-files/bs/BsBoxArrowUpRight";
import { BsPerson } from "@react-icons/all-files/bs/BsPerson";
import { BsStar } from "@react-icons/all-files/bs/BsStar";
import { BsLink45Deg } from "@react-icons/all-files/bs/BsLink45deg";
import { MdPhone } from "@react-icons/all-files/md/MdPhone";
import { MdEmail } from "@react-icons/all-files/md/MdEmail";
import { MdLocationOn } from "@react-icons/all-files/md/MdLocationOn";
import { MdLanguage } from "@react-icons/all-files/md/MdLanguage";
import { MdDirections } from "@react-icons/all-files/md/MdDirections";
import { MdRestaurantMenu } from "@react-icons/all-files/md/MdRestaurantMenu";
import { MdShare } from "@react-icons/all-files/md/MdShare";
import { MdPerson } from "@react-icons/all-files/md/MdPerson";
import { MdStar } from "@react-icons/all-files/md/MdStar";
import { MdLink } from "@react-icons/all-files/md/MdLink";
import { RiPhoneLine } from "@react-icons/all-files/ri/RiPhoneLine";
import { RiMailLine } from "@react-icons/all-files/ri/RiMailLine";
import { RiMapPinLine } from "@react-icons/all-files/ri/RiMapPinLine";
import { RiGlobalLine } from "@react-icons/all-files/ri/RiGlobalLine";
import { RiDirectionLine } from "@react-icons/all-files/ri/RiDirectionLine";
import { RiRestaurantLine } from "@react-icons/all-files/ri/RiRestaurantLine";
import { RiShareLine } from "@react-icons/all-files/ri/RiShareLine";
import { RiUserLine } from "@react-icons/all-files/ri/RiUserLine";
import { RiStarLine } from "@react-icons/all-files/ri/RiStarLine";
import { RiLinksLine } from "@react-icons/all-files/ri/RiLinksLine";
import { HiOutlinePhone } from "@react-icons/all-files/hi/HiOutlinePhone";
import { HiOutlineMail } from "@react-icons/all-files/hi/HiOutlineMail";
import { HiOutlineLocationMarker } from "@react-icons/all-files/hi/HiOutlineLocationMarker";
import { HiOutlineGlobe } from "@react-icons/all-files/hi/HiOutlineGlobe";
import { HiOutlineMenu } from "@react-icons/all-files/hi/HiOutlineMenu";
import { HiOutlineShare } from "@react-icons/all-files/hi/HiOutlineShare";
import { HiOutlineUser } from "@react-icons/all-files/hi/HiOutlineUser";
import { HiOutlineStar } from "@react-icons/all-files/hi/HiOutlineStar";
import { HiOutlineLink } from "@react-icons/all-files/hi/HiOutlineLink";
import { AiOutlinePhone } from "@react-icons/all-files/ai/AiOutlinePhone";
import { AiOutlineMail } from "@react-icons/all-files/ai/AiOutlineMail";
import { AiOutlineEnvironment } from "@react-icons/all-files/ai/AiOutlineEnvironment";
import { AiOutlineGlobal } from "@react-icons/all-files/ai/AiOutlineGlobal";
import { AiOutlineCompass } from "@react-icons/all-files/ai/AiOutlineCompass";
import { AiOutlineMenu } from "@react-icons/all-files/ai/AiOutlineMenu";
import { AiOutlineShareAlt } from "@react-icons/all-files/ai/AiOutlineShareAlt";
import { AiOutlineUser } from "@react-icons/all-files/ai/AiOutlineUser";
import { AiOutlineStar } from "@react-icons/all-files/ai/AiOutlineStar";
import { AiOutlineLink } from "@react-icons/all-files/ai/AiOutlineLink";
import { CgPhone } from "@react-icons/all-files/cg/CgPhone";
import { CgMail } from "@react-icons/all-files/cg/CgMail";
import { CgPin } from "@react-icons/all-files/cg/CgPin";
import { CgGlobeAlt } from "@react-icons/all-files/cg/CgGlobeAlt";
import { CgArrowTopRight } from "@react-icons/all-files/cg/CgArrowTopRight";
import { CgMenu } from "@react-icons/all-files/cg/CgMenu";
import { CgShare } from "@react-icons/all-files/cg/CgShare";
import { CgUser } from "@react-icons/all-files/cg/CgUser";
import { CgComment } from "@react-icons/all-files/cg/CgComment";
import { CgLink } from "@react-icons/all-files/cg/CgLink";
import { GrPhone } from "@react-icons/all-files/gr/GrPhone";
import { GrMail } from "@react-icons/all-files/gr/GrMail";
import { GrLocation } from "@react-icons/all-files/gr/GrLocation";
import { GrGlobe } from "@react-icons/all-files/gr/GrGlobe";
import { GrDirections } from "@react-icons/all-files/gr/GrDirections";
import { GrRestaurant } from "@react-icons/all-files/gr/GrRestaurant";
import { GrShareOption } from "@react-icons/all-files/gr/GrShareOption";
import { GrUser } from "@react-icons/all-files/gr/GrUser";
import { GrStar } from "@react-icons/all-files/gr/GrStar";
import { GrLink } from "@react-icons/all-files/gr/GrLink";
import { GoDeviceMobile } from "@react-icons/all-files/go/GoDeviceMobile";
import { GoMail } from "@react-icons/all-files/go/GoMail";
import { GoLocation } from "@react-icons/all-files/go/GoLocation";
import { GoGlobe } from "@react-icons/all-files/go/GoGlobe";
import { GoArrowUp } from "@react-icons/all-files/go/GoArrowUp";
import { GoThreeBars } from "@react-icons/all-files/go/GoThreeBars";
import { GoRepoForked } from "@react-icons/all-files/go/GoRepoForked";
import { GoPerson } from "@react-icons/all-files/go/GoPerson";
import { GoStar } from "@react-icons/all-files/go/GoStar";
import { GoLink } from "@react-icons/all-files/go/GoLink";
import type { IconType } from "@react-icons/all-files";
export type SystemGlyph = "PHONE"|"EMAIL"|"LOCATION"|"WEBSITE"|"DIRECTIONS"|"MENU"|"SHARE"|"CONTACT"|"REVIEW"|"LINK";
export const iconSets = {
FEATHER:{PHONE:FiPhone,EMAIL:FiMail,LOCATION:FiMapPin,WEBSITE:FiGlobe,DIRECTIONS:FiNavigation,MENU:FiMenu,SHARE:FiShare2,CONTACT:FiUser,REVIEW:FiStar,LINK:FiLink},
IONIC:{PHONE:IoCallOutline,EMAIL:IoMailOutline,LOCATION:IoLocationOutline,WEBSITE:IoGlobeOutline,DIRECTIONS:IoNavigateOutline,MENU:IoRestaurantOutline,SHARE:IoShareSocialOutline,CONTACT:IoPersonOutline,REVIEW:IoStarOutline,LINK:IoLinkOutline},
BOOTSTRAP:{PHONE:BsPhone,EMAIL:BsEnvelope,LOCATION:BsGeoAlt,WEBSITE:BsWindow,DIRECTIONS:BsCursor,MENU:BsList,SHARE:BsBoxArrowUpRight,CONTACT:BsPerson,REVIEW:BsStar,LINK:BsLink45Deg},
MATERIAL:{PHONE:MdPhone,EMAIL:MdEmail,LOCATION:MdLocationOn,WEBSITE:MdLanguage,DIRECTIONS:MdDirections,MENU:MdRestaurantMenu,SHARE:MdShare,CONTACT:MdPerson,REVIEW:MdStar,LINK:MdLink},
REMIX:{PHONE:RiPhoneLine,EMAIL:RiMailLine,LOCATION:RiMapPinLine,WEBSITE:RiGlobalLine,DIRECTIONS:RiDirectionLine,MENU:RiRestaurantLine,SHARE:RiShareLine,CONTACT:RiUserLine,REVIEW:RiStarLine,LINK:RiLinksLine},
HERO:{PHONE:HiOutlinePhone,EMAIL:HiOutlineMail,LOCATION:HiOutlineLocationMarker,WEBSITE:HiOutlineGlobe,DIRECTIONS:HiOutlineLocationMarker,MENU:HiOutlineMenu,SHARE:HiOutlineShare,CONTACT:HiOutlineUser,REVIEW:HiOutlineStar,LINK:HiOutlineLink},
ANT:{PHONE:AiOutlinePhone,EMAIL:AiOutlineMail,LOCATION:AiOutlineEnvironment,WEBSITE:AiOutlineGlobal,DIRECTIONS:AiOutlineCompass,MENU:AiOutlineMenu,SHARE:AiOutlineShareAlt,CONTACT:AiOutlineUser,REVIEW:AiOutlineStar,LINK:AiOutlineLink},
CSSGG:{PHONE:CgPhone,EMAIL:CgMail,LOCATION:CgPin,WEBSITE:CgGlobeAlt,DIRECTIONS:CgArrowTopRight,MENU:CgMenu,SHARE:CgShare,CONTACT:CgUser,REVIEW:CgComment,LINK:CgLink},
GROMMET:{PHONE:GrPhone,EMAIL:GrMail,LOCATION:GrLocation,WEBSITE:GrGlobe,DIRECTIONS:GrDirections,MENU:GrRestaurant,SHARE:GrShareOption,CONTACT:GrUser,REVIEW:GrStar,LINK:GrLink},
OCTICONS:{PHONE:GoDeviceMobile,EMAIL:GoMail,LOCATION:GoLocation,WEBSITE:GoGlobe,DIRECTIONS:GoArrowUp,MENU:GoThreeBars,SHARE:GoRepoForked,CONTACT:GoPerson,REVIEW:GoStar,LINK:GoLink}
} satisfies Record<string,Record<SystemGlyph,IconType>>;
const filledSets = {IONIC:{PHONE:IoCall,EMAIL:IoMail,LOCATION:IoLocation,WEBSITE:IoGlobe,DIRECTIONS:IoNavigate,MENU:IoRestaurant,SHARE:IoShareSocial,CONTACT:IoPerson,REVIEW:IoStar,LINK:IoLink},
REMIX:{PHONE:RiPhoneFill,EMAIL:RiMailFill,LOCATION:RiMapPinFill,WEBSITE:RiGlobalFill,DIRECTIONS:RiDirectionFill,MENU:RiRestaurantFill,SHARE:RiShareFill,CONTACT:RiUserFill,REVIEW:RiStarFill,LINK:RiLinksFill},
HERO:{PHONE:HiPhone,EMAIL:HiMail,LOCATION:HiLocationMarker,WEBSITE:HiGlobe,DIRECTIONS:HiLocationMarker,MENU:HiMenu,SHARE:HiShare,CONTACT:HiUser,REVIEW:HiStar,LINK:HiLink}} satisfies Record<string,Record<SystemGlyph,IconType>>;
export function SystemIcon({set="FEATHER",style="OUTLINE",glyph,className}:{set?:string;style?:string;glyph:SystemGlyph;className?:string}) {
  const family=Object.prototype.hasOwnProperty.call(iconSets,set)?iconSets[set as keyof typeof iconSets]:iconSets.FEATHER;
  const Icon=style==="FILLED"&&Object.prototype.hasOwnProperty.call(filledSets,set)?filledSets[set as keyof typeof filledSets][glyph]:family[glyph];
  return <Icon aria-hidden="true" data-system-icon="true" className={className}/>;
}
